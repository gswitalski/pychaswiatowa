...
> Zobacz też [README.md](./README.md) po skrócony "szybki start" z komendami oraz [strategy.md](./strategy.md) po strategię testów na poziomie projektu.
...
6. [CI/CD](#cicd)
7. [Status środowiska i przykładowe testy](#status-środowiska-i-przykładowe-testy)
...
## Status środowiska i przykładowe testy

- ✅ Vitest skonfigurowany i przetestowany
- ✅ Playwright skonfigurowany (Chromium)
- ✅ Coverage skonfigurowane (progi: 70%)

Przykładowe testy referencyjne w repozytorium:

- `src/app/app.spec.ts` - test komponentu głównego
- `src/app/core/services/auth.service.spec.ts` - test serwisu (7 testów: `signUp`, `signIn`, `signOut`, `getSession`)
- `e2e/login.spec.ts` - test strony logowania z Page Object Model

Historię konfiguracji środowiska testowego (co i kiedy zainstalowano) znajdziesz w [setup-history.md](./setup-history.md).
