...
-   *Szczegóły przepisu publicznego:* Gość może wyświetlić pełne szczegóły publicznego przepisu (nazwa, opis, zdjęcie, składniki, kroki, kategoria, tagi).
-   *CTA do rejestracji/logowania:* Elementy wymagające konta (np. dodawanie do kolekcji, edycja) nie są dostępne dla gościa i są zastąpione czytelną zachętą do zalogowania/rejestracji.
-   *Separacja od prywatnych zasobów:* Gość nie ma dostępu do przepisów `Prywatny` ani `Współdzielony`.
-   *Tryb zalogowanego na widokach publicznych:* Te same publiczne ścieżki (`/`, `/explore`, `/explore/recipes/:id-:slug`) są dostępne również dla użytkownika zalogowanego, ale interfejs dostosowuje się do jego kontekstu:
...
-   *Widoczność Sidebara:* Sidebar jest widoczny **wyłącznie** w prywatnych sekcjach aplikacji: `/dashboard`, `/recipes/**`, `/collections/**`, `/settings/**`. Na pozostałych ścieżkach (np. publiczne: `/`, `/explore`, auth: `/login`, `/register`) Sidebar nie jest wyświetlany.
...
-   *Kryteria akceptacji:*
    ...
    5.  Sidebar jest widoczny wyłącznie na ścieżkach: `/dashboard`, `/recipes/**`, `/collections/**`, `/settings/**`.
