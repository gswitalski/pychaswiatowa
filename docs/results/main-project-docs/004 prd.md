...
-   *Wyszukiwanie:* Aplikacja posiada wyszukiwarkę tekstową, która przeszukuje nazwy przepisów, listę składników oraz przypisane tagi.
    ...
    -   *Domyślna kolejność wyników (MVP):* sortowanie po **najlepszym dopasowaniu** (relevance) z priorytetem:
        ...
        -   dopasowanie w **tagach** (najniższy priorytet).
        Technicznie: punktacja ważona **3/2/1** (nazwa/składniki/tagi) i sortowanie malejąco po wyniku; przy remisie sortowanie stabilne np. `created_at.desc`.
...
-   *Wyszukiwanie publiczne (MVP):* Gość może wyszukiwać publiczne przepisy **wyłącznie tekstowo** (bez filtrów) w oparciu o nazwę, składniki i tagi, z domyślnym sortowaniem po **najlepszym dopasowaniu** (priorytet: nazwa → składniki → tagi).
