...
-   *Widoczność Sidebara:* Sidebar jest widoczny **wyłącznie** w prywatnych sekcjach aplikacji: `/dashboard`, `/my-recipies` (alias: `/my-recipes`), `/recipes/**`, `/collections/**`, `/shopping/**`, `/settings/**`. Na pozostałych ścieżkach (np. publiczne: `/`, `/explore`, auth: `/login`, `/register`) Sidebar nie jest wyświetlany.
-   *Główna nawigacja w Topbarze (desktop-first):* Topbar zawiera **stałą** (niezależną od widoku) główną nawigację aplikacji z trzema pozycjami: `Odkrywaj przepisy` (`/explore`), `Moja Pycha` (`/dashboard`) oraz `Zakupy` (`/shopping`). Aktywna pozycja jest zawsze wyróżniona.
-   *Główna nawigacja na mobile/tablet (Bottom Bar):* Dla urządzeń mobilnych i tabletów (breakpoint ~ `< 960px`) główna nawigacja **znika z góry** (nie używamy hamburgera/drawera dla menu głównego) i jest dostępna jako **Bottom Bar** przypięty na dole ekranu:
    -   3 pozycje w formie ikon (z etykietą): `Odkrywaj` (`/explore`), `Moja Pycha` (`/dashboard`), `Zakupy` (`/shopping`),
    -   aktywna pozycja jest wyróżniona,
    -   Bottom Bar jest stały (widoczny na wszystkich widokach na mobile/tablet), a treść widoku ma zapewniony odpowiedni `padding-bottom` + obsługę safe-area (aby nie zasłaniać kontentu).
-   *Stopka globalna (Footer):* Aplikacja posiada globalną stopkę widoczną na wszystkich stronach głównych i podstronach.
    -   *Zakres widoczności (MVP):* stopka jest widoczna w częściach publicznych, prywatnych oraz na widokach auth (np. `/login`, `/register`, `/email-confirmed`, `/email-confirmation-invalid`). Widok techniczny `/auth/callback` może nie wyświetlać stopki (krótki „loader/handler”).
    -   *Zawartość (MVP):*
        -   informacja o prawach autorskich w formacie: **`© {bieżący_rok} PychaŚwiatowa. Wszelkie prawa zastrzeżone.`** (rok wyliczany automatycznie),
        -   linki do stron ze statycznym tekstem:
            -   **Warunki korzystania (Regulamin):** `/legal/terms`
            -   **Polityka prywatności:** `/legal/privacy`
            -   **Wydawca serwisu:** `/legal/publisher`
    -   *Zachowanie na mobile/tablet:* stopka jest elementem treści (nie-sticky). Layouty muszą uwzględniać przypięty Bottom Bar (brak „wchodzenia” stopki pod Bottom Bar; poprawny `padding-bottom` / safe-area).
    -   *Dostępność:* linki w stopce mają poprawne etykiety, kolejność tabulacji i kontrast zgodny z resztą UI.
...
-   *Sekcja administracyjna (MVP — placeholder):* Dla użytkownika z rolą `admin` w UI dostępna jest sekcja `/admin/*`:
    -   desktop-first: w Topbarze widoczna jest pozycja **„Admin”** prowadząca do `/admin/dashboard`,
    -   mobile/tablet: **nie dodajemy** nowej pozycji do Bottom Bar; „Admin” jest dostępny jako fallback w menu użytkownika,
    -   wejście na `/admin/*` jest chronione guardem po roli (`app_role === 'admin'`), a brak dostępu skutkuje przekierowaniem na `/forbidden`,
    -   pozycja „Admin” nie może „migać” — jest renderowana dopiero po ustaleniu roli (bootstrap np. `GET /me`).
...
-   *Rozbudowany panel administracyjny:* Kompleksowy panel do zarządzania aplikacją (listy użytkowników, moderacja, konfiguracja, audyt/log). Początkowe zadania administracyjne będą wykonywane bezpośrednio w bazie danych.
    -   **Uwaga (w zakresie MVP):** istnieje tylko przygotowana sekcja `/admin/*` z placeholderem dashboardu oraz ochroną dostępu po roli `admin` (bez realnych narzędzi administracyjnych).
...
-   *Kryteria akceptacji:*
    1.  Aplikacja posiada stały pasek boczny (Sidebar) zawierający linki wyłącznie do głównych modułów prywatnych: Moja Pycha, Przepisy, Kolekcje, Zakupy, Ustawienia.
    2.  Pasek boczny nie zawiera przycisków operacyjnych (takich jak "Dodaj przepis").
    3.  Topbar zawiera stałą główną nawigację (zakładki) z pozycjami: `Odkrywaj przepisy` (`/explore`), `Moja Pycha` (`/dashboard`) oraz `Zakupy` (`/shopping`) (desktop-first).
    ...
    7.  Sidebar jest widoczny wyłącznie na ścieżkach: `/dashboard`, `/my-recipies` (alias: `/my-recipes`), `/recipes/**`, `/collections/**`, `/shopping/**`, `/settings/**`.
    8.  Na urządzeniach mobilnych i tabletach (breakpoint ~ `< 960px`) główna nawigacja jest dostępna w formie **Bottom Bar** przypiętego na dole ekranu (bez hamburgera/drawera dla menu głównego) i zachowuje tę samą kolejność oraz etykiety:
        - `Odkrywaj przepisy` (`/explore`)
        - `Moja Pycha` (`/dashboard`)
        - `Zakupy` (`/shopping`)
    9.  Bottom Bar jest widoczny na wszystkich widokach na mobile/tablet (w tym na publicznych `/` i `/explore`) i nie zasłania treści (zapewniony `padding-bottom` + safe-area).
    10. Dla gościa (niezalogowanego) kliknięcie w pozycje wymagające logowania (`/dashboard`, `/shopping`) przekierowuje do logowania z zachowaniem powrotu do docelowej ścieżki po sukcesie.

---
-   *ID:* US-056
-   *Tytuł:* Mobilna nawigacja dolna (Bottom Bar) zamiast hamburgera
-   *Opis:* Jako użytkownik korzystający z aplikacji na telefonie lub tablecie, chcę mieć główne menu jako pasek ikon przypięty na dole ekranu, abym szybciej przełączał się między kluczowymi modułami bez rozwijania hamburgera.
-   *Kryteria akceptacji:*
    1.  Dla szerokości ekranu odpowiadającej mobile/tablet (breakpoint ~ `< 960px`) główna nawigacja w Topbarze nie jest wyświetlana jako zakładki.
    2.  Na dole ekranu widoczny jest przypięty Bottom Bar z trzema pozycjami: `Odkrywaj` (`/explore`), `Moja Pycha` (`/dashboard`), `Zakupy` (`/shopping`).
    3.  Każda pozycja Bottom Bara jest prezentowana jako ikonka + etykieta, a aktywna pozycja jest zawsze wyróżniona.
    4.  Bottom Bar jest dostępny na wszystkich widokach na mobile/tablet, w tym na publicznych ścieżkach `/` i `/explore`.
    5.  Kliknięcie w `Moja Pycha` lub `Zakupy` jako gość (niezalogowany) przekierowuje do logowania i po zalogowaniu odtwarza nawigację do pierwotnie wybranej ścieżki (returnUrl).
    6.  Bottom Bar nie zasłania treści: widoki mają zapewniony `padding-bottom` oraz obsługę bezpiecznego obszaru (safe-area) dla urządzeń z gestami.
    7.  Bottom Bar nie zastępuje akcji kontekstowych: przyciski akcji nadal znajdują się w Page Headerze i działają bez zmian.
...
---
-   *ID:* US-057
-   *Tytuł:* Globalna stopka i strony informacyjne (Regulamin / Polityka prywatności / Wydawca)
-   *Opis:* Jako użytkownik (gość lub zalogowany), chcę mieć stały dostęp do stopki z informacjami prawnymi, abym mógł szybko znaleźć regulamin i politykę prywatności niezależnie od tego, na jakiej stronie jestem.
-   *Kryteria akceptacji:*
    1.  Stopka jest widoczna na wszystkich stronach głównych i podstronach w aplikacji (publicznych, prywatnych i auth), z wyjątkiem technicznego widoku `/auth/callback`, który może nie wyświetlać stopki.
    2.  Stopka zawiera informację o prawach autorskich w formacie: `© {bieżący_rok} PychaŚwiatowa. Wszelkie prawa zastrzeżone.` (rok wyliczany automatycznie).
    3.  Stopka zawiera linki do trzech stron:
        - `Warunki korzystania (Regulamin)` → `/legal/terms`
        - `Polityka prywatności` → `/legal/privacy`
        - `Wydawca serwisu` → `/legal/publisher`
    4.  Linki w stopce działają jako nawigacja SPA (bez przeładowania strony) i są dostępne zarówno dla gościa, jak i użytkownika zalogowanego (bez wymogu logowania).
    5.  Strony `/legal/terms`, `/legal/privacy`, `/legal/publisher` w MVP mogą prezentować **placeholder** treści (właściwe teksty zostaną dostarczone osobno); istotne jest istnienie routingu i stabilnych ścieżek URL.
    6.  Na mobile/tablet stopka nie jest zasłonięta przez przypięty Bottom Bar (layout zapewnia odpowiedni `padding-bottom` / safe-area).
    7.  Stopka spełnia podstawowe wymagania dostępności: poprawna kolejność tabulacji i jednoznaczne etykiety linków.

---
-   *ID:* US-058
-   *Tytuł:* Dostęp do sekcji administracyjnej (Admin) z nawigacji
-   *Opis:* Jako użytkownik z rolą `admin`, chcę widzieć pozycję „Admin” w nawigacji, aby szybko przejść do panelu administracyjnego.
-   *Kryteria akceptacji:*
    1.  Po zalogowaniu jako `admin` widzę pozycję „Admin” w Topbarze (desktop-first).
    2.  Na mobile/tablet nie dodajemy nowej pozycji do Bottom Bar; „Admin” jest dostępny z menu użytkownika.
    3.  Kliknięcie „Admin” prowadzi do `/admin/dashboard`.
    4.  Widok `/admin/dashboard` prezentuje placeholder (nagłówek + 2–4 karty „Wkrótce”) spójny z Angular Material.
    5.  Pozycja „Admin” nie pojawia się przed ustaleniem roli (brak migania).

---
-   *ID:* US-059
-   *Tytuł:* Blokada dostępu do `/admin/*` dla nie-admina
-   *Opis:* Jako użytkownik bez roli `admin`, chcę aby aplikacja blokowała dostęp do sekcji administracyjnej, aby nie było możliwości podejrzenia narzędzi administracyjnych.
-   *Kryteria akceptacji:*
    1.  Jeśli zalogowany użytkownik bez roli `admin` wejdzie na `/admin` lub `/admin/dashboard`, następuje przekierowanie na `/forbidden`.
    2.  Jeśli użytkownik niezalogowany wejdzie na `/admin/*`, działa standardowa ochrona tras prywatnych (np. przekierowanie na `/login`), zgodna z istniejącym mechanizmem.
