...
-   *Kryteria akceptacji:*
    1.  Formularz dodawania przepisu zawiera pola na: nazwę (tekst), opis (tekst), **liczba porcji (opcjonalnie)**, **czas przygotowania (opcjonalnie, w minutach)**, **czas całkowity (opcjonalnie, w minutach)**, **flaga "Termorobot" (checkbox/toggle, opcjonalnie, domyślnie wyłączona)**, zdjęcie (przesyłanie pliku), składniki (`textarea`), kroki (`textarea`).
...
-   *Kryteria akceptacji:*
    ...
    10. Jeśli przepis ma uzupełniony czas przygotowania, jest on wyświetlany **pod opisem** jako metadana z ikonką (np. `schedule`) oraz wartością w minutach (formatowaną w UI).
    11. Jeśli przepis ma uzupełniony czas całkowity, jest on wyświetlany **pod opisem** jako metadana z ikonką (np. `timer`) oraz wartością w minutach (formatowaną w UI).
    12. Jeśli dana wartość czasu nie jest ustawiona, odpowiadająca jej metadana nie jest wyświetlana.
