...
-   *Aktualizacja (Update):* Użytkownik może edytować każdy element zapisanego przepisu (nazwę, opis, poszczególne składniki i kroki, zdjęcie, widoczność). Edycja odbywa się na ustrukturyzowanych danych, a nie na surowym tekście.
...
-   *Widoczność Sidebara:* Sidebar jest widoczny **wyłącznie** w prywatnych sekcjach aplikacji: `/dashboard`, `/my-recipes`, `/recipes/**`, `/collections/**`, `/settings/**`. Na pozostałych ścieżkach (np. publiczne: `/`, `/explore`, auth: `/login`, `/register`) Sidebar nie jest wyświetlany.
...
-   *Kryteria akceptacji:*
    1.  Dostępna jest chroniona strona "Moje przepisy" pod ścieżką `/my-recipes`, która wyświetla wszystkie przepisy zalogowanego użytkownika.
    ...
    5.  Wejście na listę przepisów jest możliwe po zalogowaniu (np. poprzez link w Sidebarze). (Opcjonalnie) ` /recipes` może przekierowywać do `/my-recipes` dla kompatybilności wstecznej.
...
-   *Kryteria akceptacji:*
    1.  Aplikacja posiada stały pasek boczny (Sidebar) zawierający linki wyłącznie do głównych modułów: Moja Pycha, Przepisy, Kolekcje, Ustawienia.
    2.  Pasek boczny nie zawiera przycisków operacyjnych (takich jak "Dodaj przepis").
    3.  W górnej części aplikacji (Topbar) widoczna jest ścieżka powrotu (Breadcrumbs), np. `Kolekcje > Święta > Sernik`, umożliwiająca powrót do widoku nadrzędnego.
    4.  Topbar zawiera globalny pasek wyszukiwania (Omnibox) dostępny z każdego ekranu.
    5.  Sidebar jest widoczny wyłącznie na ścieżkach: `/dashboard`, `/my-recipes`, `/recipes/**`, `/collections/**`, `/settings/**`.
...
-   *Opis:* Jako użytkownik zalogowany, chcę mieć możliwość przejścia do widoku "Moja Pycha" bezpośrednio z górnego paska (Topbar), aby szybko wracać do mojego punktu startowego.
-   *Kryteria akceptacji:*
    1.  W Topbarze, po lewej stronie ikonki użytkownika (avatar), znajduje się element głównej nawigacji poziomej "Moja Pycha".
    2.  Kliknięcie "Moja Pycha" przenosi użytkownika na ścieżkę `/dashboard`.
    3.  Etykieta "Moja Pycha" zastępuje w UI wcześniejszą nazwę "Dashboard" (route i identyfikatory techniczne mogą pozostać bez zmian).
