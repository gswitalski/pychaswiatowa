...
-   *Reguły scalania (deduplikacja):* Pozycje z przepisów są scalane w jeden element listy zakupów, jeśli mają identyczne:
    -   `nazwa` (ze składników znormalizowanych),
    -   `jednostka` (w tym przypadek `jednostka = null`).
    Dla pozycji scalonych:
    -   jeśli `jednostka != null` i `ilosc != null`, ilości są sumowane,
    -   jeśli `jednostka = null` (lub brak ilości), na liście istnieje pojedyncza pozycja „tylko nazwa” (bez sumowania ilości).
    -   *Przykład:* jeśli w planie są przepisy z: `cukier 200 g`, `cukier 50 g`, `cukier 1 łyżeczka`, `cukier` (bez ilości/jednostki), to na liście widnieje: `cukier, 250 g`; `cukier, 1 łyżeczka`; `cukier` (bez ilości).
-   *Odhaczanie posiadanych:* Użytkownik może oznaczyć pozycję jako „posiadane”.
    ...
    -   stan posiadania jest zapisywany per pozycja listy zakupów.
...
-   *Spójność z planem:*
    -   dodanie przepisu do „Mojego planu” dodaje jego składniki znormalizowane do listy zakupów wg reguł scalania,
    -   usunięcie przepisu z „Mojego planu” usuwa odpowiednie ilości składników (odejmuje wkład przepisu) z listy zakupów,
    -   dla pozycji z ilością/jednostką, jeśli po odjęciu ilość wynosi `0`, pozycja jest usuwana z listy,
...
-   *Kryteria akceptacji:*
    ...
    2.  Pozycje z przepisów są scalane wg klucza (`nazwa`, `jednostka`); dla `jednostka != null` i `ilosc != null` ilości są sumowane.
    3.  Pozycje, dla których nie da się określić ilości/jednostki (tj. `jednostka = null`), pojawiają się jako pojedyncza pozycja „tylko nazwa” i nie sumują ilości.
    4.  System nie scala ręcznych pozycji użytkownika z pozycjami z przepisów (MVP).
...
-   *Kryteria akceptacji:*
    1.  Po usunięciu przepisu z planu system odejmuje jego wkład składników znormalizowanych od listy zakupów (dla pozycji z `ilosc/jednostka`).
    2.  Jeśli po odjęciu ilość danej pozycji wynosi `0`, pozycja jest usuwana z listy zakupów.
    3.  Ręczne pozycje dodane przez użytkownika nie są usuwane ani modyfikowane przez operację usuwania przepisu z planu.
...
-   *Kryteria akceptacji:*
    ...
    3.  Stan „posiadane” jest zapisywany i odtwarzany po ponownym wejściu na widok „Zakupy”.
