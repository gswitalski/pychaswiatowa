-- migration: update create_recipe_with_tags for tips parameter
-- description: adds p_tips_raw parameter to create_recipe_with_tags function
-- functions affected: create_recipe_with_tags
-- note: tips_raw is optional (defaults to null), empty values result in '[]'::jsonb

-- update create_recipe_with_tags function to include tips parameter
...
begin
    ...
    -- NEW: parse tips to jsonb (empty if null or empty string)
    ...
    insert into public.recipes (
        ...
        v_parsed_steps,
        v_parsed_tips,          -- NEW: parsed tips value
        p_visibility,
        p_servings,
        p_is_termorobot,
        p_prep_time_minutes,
...
comment on function public.create_recipe_with_tags(uuid, text, text, bigint, text, text, text[], recipe_visibility, smallint, boolean, smallint, smallint, recipe_diet_type, recipe_cuisine, recipe_difficulty, boolean, text) is
    'Creates a new recipe with associated tags, visibility setting, servings, termorobot flag, time fields, classification fields (diet_type, cuisine, difficulty, is_grill), tips, in a single atomic transaction. Returns the new recipe ID.';
