-- migration: update update_recipe_with_tags function for classification fields
-- description: adds diet_type, cuisine, and difficulty parameters to update_recipe_with_tags function
-- functions affected: update_recipe_with_tags
-- note: uses update flags to distinguish between "not provided" and "set to null"

-- update update_recipe_with_tags function to include classification parameters
create or replace function public.update_recipe_with_tags(
    ...
    p_diet_type public.recipe_diet_type default null,      -- NEW: diet type parameter
    p_update_diet_type boolean default false,              -- NEW: update diet type flag
    p_cuisine public.recipe_cuisine default null,          -- NEW: cuisine parameter
    p_update_cuisine boolean default false,                -- NEW: update cuisine flag
    p_difficulty public.recipe_difficulty default null,    -- NEW: difficulty parameter
    p_update_difficulty boolean default false              -- NEW: update difficulty flag
)
returns bigint
language plpgsql
security definer
...
begin
    ...
    set
        ...
        diet_type = case                                    -- NEW: conditional diet type update
            when p_update_diet_type then p_diet_type
            else diet_type
        end,
        cuisine = case                                      -- NEW: conditional cuisine update
            when p_update_cuisine then p_cuisine
            else cuisine
        end,
        difficulty = case                                   -- NEW: conditional difficulty update
...
comment on function public.update_recipe_with_tags(bigint, uuid, text, text, bigint, text, text, text[], boolean, recipe_visibility, text, boolean, smallint, boolean, boolean, boolean, smallint, boolean, smallint, boolean, recipe_diet_type, boolean, recipe_cuisine, boolean, recipe_difficulty, boolean) is
    'Updates an existing recipe with associated tags, visibility, image path, category, servings, termorobot flag, time fields, and classification fields (diet_type, cuisine, difficulty) in a single atomic transaction. Returns the updated recipe ID.';

-- grant execute permission to authenticated users
grant execute on function public.update_recipe_with_tags(bigint, uuid, text, text, bigint, text, text, text[], boolean, recipe_visibility, text, boolean, smallint, boolean, boolean, boolean, smallint, boolean, smallint, boolean, recipe_diet_type, boolean, recipe_cuisine, boolean, recipe_difficulty, boolean) to authenticated;
