-- migration: update update_recipe_with_tags to reset normalized ingredients status
-- description: ensures updated recipes reset normalized_ingredients_status to 'PENDING' when ingredients change
-- functions affected: update_recipe_with_tags
-- note: status is reset to PENDING (and updated_at to NULL) whenever ingredients are updated

-- update update_recipe_with_tags function to reset normalized ingredients status
create or replace function public.update_recipe_with_tags(
    ...
    p_update_tips boolean default false
)
returns bigint
language plpgsql
security definer
...
declare
    ...
    v_ingredients_updated boolean := false;  -- NEW: track if ingredients are updated
begin
    ...
    if p_ingredients_raw is not null then
        ...
        v_ingredients_updated := true;  -- NEW: mark ingredients as updated
    ...
    -- parse tips if update flag is set
    ...
    set
        ...
        -- NEW: reset normalized ingredients status if ingredients changed
        normalized_ingredients_status = case
            when v_ingredients_updated then 'PENDING'
            else normalized_ingredients_status
        end,
        normalized_ingredients_updated_at = case
            when v_ingredients_updated then null
            else normalized_ingredients_updated_at
        end,
        updated_at = now()
    where id = p_recipe_id
      and user_id = p_user_id
...
comment on function public.update_recipe_with_tags(bigint, uuid, text, text, bigint, text, text, text[], boolean, recipe_visibility, text, boolean, smallint, boolean, boolean, boolean, smallint, boolean, smallint, boolean, recipe_diet_type, boolean, recipe_cuisine, boolean, recipe_difficulty, boolean, boolean, boolean, text, boolean) is
    'Updates an existing recipe with associated tags, visibility, image path, category, servings, termorobot flag, time fields, classification fields (diet_type, cuisine, difficulty, is_grill), and tips in a single atomic transaction. Resets normalized_ingredients_status to PENDING if ingredients are updated. Returns the updated recipe ID.';
