-- migration: update get_recipes_list for time fields
-- description: adds prep_time_minutes and total_time_minutes return columns to get_recipes_list function
-- functions affected: get_recipes_list
-- note: returns time fields in recipe lists
...
returns table (
    ...
    prep_time_minutes smallint,      -- NEW: prep time column
    total_time_minutes smallint,     -- NEW: total time column
    total_count bigint
)
language plpgsql
security definer
...
begin
    ...
    return query execute format('
        ...
        select
            ...
            cr.servings,
            cr.is_termorobot,
            cr.prep_time_minutes,      -- NEW: prep time column
            cr.total_time_minutes,     -- NEW: total time column
            cr.total_count
...
comment on function public.get_recipes_list(uuid, text, integer, integer, text, text, bigint, bigint[], text, boolean) is
    'Retrieves a paginated list of recipes with support for owned and my_recipes views. Returns recipe details with ownership, collection status, servings, termorobot flag, and time fields (prep_time_minutes, total_time_minutes). Supports filtering by termorobot flag.';
