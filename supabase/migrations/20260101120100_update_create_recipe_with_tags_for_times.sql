-- migration: update create_recipe_with_tags function for time fields
-- description: adds prep_time_minutes and total_time_minutes parameters to create_recipe_with_tags function
-- functions affected: create_recipe_with_tags
-- note: implements cross-field validation for time fields

-- update create_recipe_with_tags function to include time parameters
...
begin
    ...
    insert into public.recipes (
        ...
        servings,
        is_termorobot,
        prep_time_minutes,      -- NEW: prep time column
        total_time_minutes      -- NEW: total time column
    ) values (
        p_user_id,
        p_name,
        p_description,
        ...
        p_servings,
        p_is_termorobot,
        p_prep_time_minutes,    -- NEW: prep time value
        p_total_time_minutes    -- NEW: total time value
    )
...
comment on function public.create_recipe_with_tags(uuid, text, text, bigint, text, text, text[], recipe_visibility, smallint, boolean, smallint, smallint) is
    'Creates a new recipe with associated tags, visibility setting, servings, termorobot flag, and time fields (prep_time_minutes, total_time_minutes) in a single atomic transaction. Returns the new recipe ID.';

-- grant execute permission to authenticated users
grant execute on function public.create_recipe_with_tags(uuid, text, text, bigint, text, text, text[], recipe_visibility, smallint, boolean, smallint, smallint) to authenticated;
