-- Migration: Create RPC for atomic add recipe to plan with shopping list update
-- Description: Atomically adds recipe to user's plan and updates shopping list based on normalized ingredients
...
begin
    ...
    if v_normalized_status = 'READY' then
        ...
        if v_normalized_items is not null and jsonb_array_length(v_normalized_items) > 0 then
            -- Process each normalized ingredient
            for v_item in select * from jsonb_array_elements(v_normalized_items)
            loop
                -- Extract fields from normalized ingredient
                v_name := v_item->>'name';
                v_unit := v_item->>'unit';
                ...
                if found then
                    ...
                    if v_unit is not null and v_amount is not null then
                        ...
                        -- For name-only items (unit=null), just touch updated_at
                        update public.shopping_list_items
                        set updated_at = now()
                        where user_id = v_user_id
                          and kind = 'RECIPE'
    ...
    return jsonb_build_object(
        ...
        'shopping_list_updated', v_normalized_status = 'READY',
...
comment on function public.add_recipe_to_plan_and_update_shopping_list(bigint) is 
    'Atomically adds recipe to user plan and updates shopping list based on normalized ingredients (if READY). Enforces access control and business rules.';
