-- migration: update get_recipes_list for grill flag
-- description: updates get_recipes_list RPC to support grill filter and return is_grill field
-- functions affected: get_recipes_list
-- dependencies: recipes (with is_grill field)
...
drop function if exists public.get_recipes_list(uuid, text, integer, integer, text, text, bigint, bigint[], text, boolean, recipe_diet_type, recipe_cuisine, recipe_difficulty, boolean);
 ...
 * @param p_cuisine - Optional cuisine filter (25 world cuisines available)
 * @param p_difficulty - Optional difficulty filter (EASY, MEDIUM, HARD)
 * @param p_grill - Optional grill filter (true, false, or null for no filter)
 *
 ...
 *   - is_grill: grill flag (non-nullable, defaults to false)
...
create or replace function public.get_recipes_list(
    ...
    p_grill boolean default null                        -- NEW: grill filter parameter
)
returns table (
    id bigint,
    name text,
    ...
    is_grill boolean,                                   -- NEW: grill flag in result
    total_count bigint
)
language plpgsql
security definer
    ...
    return query execute format('
        with filtered_recipes as (
            ...
            where r.deleted_at is null
              ...
              -- Grill filter (NEW)
              and ($10::boolean is null or r.is_grill = $10)
        ),
        counted_recipes as (
            select
                fr.*,
...
comment on function public.get_recipes_list(uuid, text, integer, integer, text, text, bigint, bigint[], text, boolean, recipe_diet_type, recipe_cuisine, recipe_difficulty, boolean) is
    'Retrieves a paginated list of recipes with support for owned and my_recipes views. Returns recipe details with ownership, collection status, and classification fields (diet_type, cuisine, difficulty, is_grill).';

-- Grant execute permission to authenticated users
grant execute on function public.get_recipes_list(uuid, text, integer, integer, text, text, bigint, bigint[], text, boolean, recipe_diet_type, recipe_cuisine, recipe_difficulty, boolean) to authenticated;
